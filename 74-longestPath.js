...
function getLongestPath(graph, start, end) {
  const queue = [start];
  const visited = [];
  const node = queue[queue.length - 1];
  let length = 0;

  if (node == end) return visited.length;

  if (visited.includes(node)) return visited.length;
  else {
    visited.push(node);
  }
  ...
  for (let next_node in graph[node]) {
    ...
    max.push(length, getLongestPath(queue, visited));
    length = Math.max(...max);
...
const longestPath = getLongestPath(graph, start, end);
